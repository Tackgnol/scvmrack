CREATE OR REPLACE FUNCTION generate_character(p_class_id integer DEFAULT NULL::integer) RETURNS uuid
    LANGUAGE plpgsql
    AS $$
DECLARE
char_id UUID;
...
str_raw := (SELECT SUM(floor(random()*6)+1) FROM generate_series(1,3)) + COALESCE((class_row.stat_modifiers->>'strength')::INTEGER, 0);
    ...
    -- 7. EQUIPMENT POOL (FIXED: Added COALESCE to prevent NULL poisoning)
    ...
    -- Class weapon rolls (FIXED: Added COALESCE)
    rolled_pool := rolled_pool || COALESCE((
    ...
    -- Class armor rolls (FIXED: Added COALESCE)
    rolled_pool := rolled_pool || COALESCE((
