-- Migration: Fix get_character_full to read tags/dice from stored equipment JSONB
-- Date: 2026-01-24
-- Issue: Equipment items were missing tags and dice because the function only looked
--        at master tables, ignoring values stored directly in the character's JSONB
...
SELECT jsonb_agg(
               jsonb_build_object(
                       ...
                       (
                           ...
                           FROM (
                                    -- First: read dice from stored equipment item
...
-- Final return with explicit casts
RETURN QUERY SELECT
        result.id,
        result.name::text,
        result.class_id,
