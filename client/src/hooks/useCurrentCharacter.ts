...
export function useCurrentCharacter() {
  ...
  const trimmedLocale =
    getApiLocale<PathsCharactersNewPostParametersQueryLocale>(locale);
  ...
  const claimCharacter = useCallback(
    async (idToClaim?: string) => {
      ...
      try {
        ...
        showError(
          `Failed to claim character: ${err instanceof Error ? err.message : 'Unknown error'}`
        );
        throw err;
      } finally {
        setIsClaiming(false);
