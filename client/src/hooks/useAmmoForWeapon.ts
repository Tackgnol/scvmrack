...
export function useAmmoForWeapon(weapon: EquipmentItem | null): AmmoInfo {
    ...
    // Intentionally compute on every render.
    // The character equipment array is sometimes mutated in-place by optimistic updates,
    // so memoizing by reference can return stale totals.
    ...
    for (let i = 0; i < equipment.length; i++) {
        ...
        if (item.ammoType === targetType && item.tags?.includes('ammo')) {
            ...
            // Ammo can be represented either as:
            // - a single stack with `amount` (legacy / some flows), or
            // - multiple inventory entries (1 per arrow/bolt), often with `amount` missing/null.
            // Also guard against stale/invalid `amount: 0` by treating a present entry as 1.
