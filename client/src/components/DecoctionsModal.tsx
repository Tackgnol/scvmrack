...
const decoctions = [
  ...
  },
  {
    ...
    fallbackDesc: 'Pass a dr14 test or severe (and arguably fun) hallucinations for d4 hours.'
  },
  {
    id: 3,
    nameKey: 'abilities.occult_herbmaster.frog',
    fallbackName: 'Southern Frog Stew',
    fallbackDesc: 'Vomit for d4 hours, pass a dr14 test or you can do nothing else.'
  },
  {
    id: 4,
  ...
  },
  {
    ...
    fallbackDesc: 'Translucent oil, must be dabbed right into the eye. Heals infection and gives +2 on Presence tests for d4 hours.'
  },
  {
    id: 7,
    nameKey: 'abilities.occult_herbmaster.hyphos',
    fallbackName: 'Hyphos\' Enervating Snuff',
    fallbackDesc: 'Berserk! Two attacks per round but defend with dr14. Lasts one fight. Must be snorted, causes sneezing.'
  },
  {
    id: 8,
...
const DecoctionsModal: React.FC<DecoctionsModalProps> = ({ open, onClose }) => {
  ...
  return (
    <MorkBorgModal
      open={open}
      onClose={onClose}
      title={t('abilities.occult_herbmaster.decoctions_title', 'Occult Herbmaster Decoctions (d8)')}
      maxWidth="sm"
    >
      <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
        {decoctions.map((decoction, index) => {
          const translatedText = t(decoction.nameKey, `${decoction.fallbackName}: ${decoction.fallbackDesc}`);
          const parts = translatedText.split(':');
          const name = parts[0] || decoction.fallbackName;
          const desc = parts.slice(1).join(':').trim() || decoction.fallbackDesc;
          ...
          return (
            <Box key={decoction.id} sx={{ mb: 1 }}>
              ...
              {index < decoctions.length - 1 && (
                <Divider sx={{ mt: 1.5, borderColor: 'rgba(255, 62, 181, 0.2)', borderStyle: 'dashed' }} />
              )}
            </Box>
          );
        })}
