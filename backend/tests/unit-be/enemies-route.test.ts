...
const { default: enemyRoutes } = await import(
  '../../src/routes/parties/enemies.js'
);

async function buildApp() {
  const app = Fastify({ logger: false });
  await app.register(errorHandlerPlugin);
  await app.register(enemyRoutes);
  await app.ready();
  return app;
}
...
test('GET by-room enemies forwards room enemy read', async () => {
...
test('room enemy read surfaces a service 404', async () => {
  ownerResult = {
    ok: false,
