import {
  ...
  type EnemyCard,
  type EnemyFull,
} from '../lib/enemy-card.js';
import {
  enemyRepository,
  type EnemyWriteData,
} from '../repositories/enemy-repository.js';
import { partyRepository } from '../repositories/party-repository.js';
import { badRequest, notFound, type ApiHttpError } from '../errors.js';
import { isValidUUID } from '../utils.js';
import { isCharacterVisibleInObrRoom } from './obr-room-visibility-service.js';
import {
  fail,
  ok,
  unexpected,
...
export function createEnemyService(log: ServiceLogger) {
  ...
  async function ensureRoomParty(
    roomId: string
  ...
  return {
    async listForOwner(input: {
      roomId: string;
    }): Promise<ServiceResult<EnemyFull[]>> {
      try {
        const owned = await ensureRoomParty(input.roomId);
    ...
    async listCardsForPlayer(input: {
      roomId: string;
      characterId: string;
    }): Promise<ServiceResult<EnemyCard[]>> {
      ...
      try {
        if (!(await isCharacterVisibleInObrRoom(input.characterId, room))) {
          return ok([]);
        }
