 ...
 */
export function sanitizeCharacterUpdate(updates: Record<string, unknown>): Record<string, unknown> {
    ...
    for (const [key, value] of Object.entries(updates)) {
        ...
        if (key in stringFields && typeof value === 'string') {
            ...
            if (Number.isFinite(value)) {
                sanitized[key] = Math.max(bounds.min, Math.min(bounds.max, Math.floor(value)));
            }
